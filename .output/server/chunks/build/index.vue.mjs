import { defineComponent, createElementBlock, openBlock, createElementVNode, warn, createVNode, h, renderSlot, ref, provide, watch, unref, createBlock, Teleport as Teleport$1, computed, mergeProps, isRef, onScopeDispose, getCurrentInstance, inject, nextTick, useSlots, withCtx, Transition, withDirectives, withModifiers, createCommentVNode, normalizeClass, toDisplayString, vShow, withAsyncContext, Fragment, renderList, useSSRContext } from 'vue';
import { NOOP, isObject, hasOwn, isString, camelize } from '@vue/shared';
import { fromPairs, isNil, isUndefined as isUndefined$1, get } from 'lodash-unified';
import { d as useNamespace, i as isElement, e as isNumber, f as isStringNumber, g as isUndefined, h as isBoolean, j as useZIndex, k as useId, l as defaultNamespace, u as useRouter } from './server.mjs';
...
import { u as useMock } from './useMockData.mjs';
...
const _sfc_main$1 = {
  ...
  async setup(__props) {
    ...
    const tabClick = (id) => {
      ...
      const scrollContainer = (void 0).scrollingElement || (void 0).documentElement;
      if (el) {
        const top = el.getBoundingClientRect().top + (void 0).pageYOffset;
...
const _imports_2 = "" + __buildAssetsURL("header_bg.AhafPLhb.png");
...
const _imports_6 = "" + __buildAssetsURL("instagram.CHSiGVFs.png");
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div></div></div></div><div id="contact" class="w-full py-60 md:py-100 relative text-14 flex justify-center" style="${ssrRenderStyle(`background-image: url('/full/download-bg.png')`)}"><div class="w-full md:w-md lg:w-lg xl:w-xl flex flex-col lg:flex-row justify-between px-16"><div class="w-full lg:w-1/2 leading-1.2"><div class="text-18 font-600">Contact Us</div><div class="text-18 md:text-[28px] leading-1.5 pt-10 text-[#8c8d9e]">${((_h = data.value.contact) == null ? void 0 : _h.name) ?? ""}</div><div class="text-18 pt-16 md:text-24 text-[#007bff]"><a class=""${ssrRenderAttr("href", data.value.emailHref)}>${ssrInterpolate(data.value.email)}</a></div></div><div class="w-full mt-10 lg:mt-0 lg:w-1/2"><img class="w-full md:w-320 jump mt-20 lg:mt-0"${ssrRenderAttr("src", `/${name.value}/contact_img.png`)}></div></div></div><div class="h-100 bg-[#222] w-full"></div><div id="screenshot" class="py-120 bg-[#eee] w-full px-8"><div class="text-center font-600 text-[222] text-30">App Screenshots</div><div class="block sm:hidden pt-70">`);
